...
{
	snakeUpdate(&g->s); /* cập nhật trạng thái rắn */
	switch (g->dir) { /* di chuyển rắn */
	case UP: g->s.coords[0].y--; break; /* lên */
	case DOWN: g->s.coords[0].y++; break; /* xuống */
	case LEFT: g->s.coords[0].x--; break; /* trái */
	case RIGHT: g->s.coords[0].x++; break; /* phải */
	}
	...
	/* cho phép rắn di chuyển qua các biên*/
...
void gameInput(GAME* g)
{
	...
	{
	case SDL_KEYDOWN:
		switch (e.key.keysym.sym) /* xác định hướng đi chuyển chống đi lùi */
		{
...
void gameLoop(GAME* g)
{
	...
	while (g->running) {
		...
		gameDraw(g); /* dựng 1 frame hình */
					 ...
					 /* tính toán dựng 1 frame hình để giới hạn frame rate */
		timerDiff = SDL_GetTicks() - timerBegin;
		timerSleep = GAME_FRAME_TIME - timerDiff;
		if (timerSleep > 0)
			SDL_Delay(timerSleep); /* chờ cho đủ thời gian 12fps */
	}
}