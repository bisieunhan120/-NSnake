...
#ifdef _MSC_VER // Tham số Visual liên kết với C++
...
int event() // xử lý sự kiện
{
	...
	int running = 1; // biến kiểm soát vòng lập
...
void done() // dọn dẹp 
{
	SDL_DestroyRenderer(rend); // hủy đối tượng dựng hình
...
int main(int argc, char** argv)
{
	...
	char buf[64]; // buffer tạm để hiện noi  dung FPS
	init(1); // khởi tạo ứng dụng Vsync bật
	...
	while (running)
	{
		...
		draw(); // dựng hình
		frameCount++; // đã dựng xong frame
					  // tính số FPS
		fps = frameCount / ((SDL_GetTicks() - timerBegin) / 1000.0f);
		// và hiện thị lên tiêu đề cửa sổ
